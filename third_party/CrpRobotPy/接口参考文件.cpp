...
PYBIND11_MODULE(CrpRobotPy, m) {
   ...
   py::class_<Crp::CrpRobot>(m, "CrpRobotPy")
      ...
      // 组输出 GOT，对应 Crp SDK IIOService::setGOT / getGOT
      .def("set_GOT", &Crp::CrpRobot::set_GOT,
         py::arg("index"),
         py::arg("value"),
         "Set a single GOT (group output) variable by index")

      .def("get_GOT", &Crp::CrpRobot::get_GOT,
         py::arg("index"),
         "Get a single GOT (group output) value; returns int64 or throws on error")
